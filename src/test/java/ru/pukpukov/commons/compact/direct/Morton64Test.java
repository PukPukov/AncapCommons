...
import ru.pukpukov.commons.compact.Morton64;
...
public class Morton64Test {
    ...
    private void doTestSPackUnpack(long dimensions, long bits, long... values) {
        Morton64 m = new Morton64(dimensions, bits);
        long code = m.signedPack(values);
        long[] unpacked = m.signedUnpack(code);
        assertArrayEquals(values, unpacked);
        
        // Test with both the "return fresh array" and "unpack into array" signatures.
        unpacked = new long[(int)dimensions];
        m.signedUnpack(code, unpacked);
        assertArrayEquals(values, unpacked);
    }
    
    @Test
    public void testSPackUnpack() {
